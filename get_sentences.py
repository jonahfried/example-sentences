...
class WikiSentenceExtractor():
    ...
    def run(self):
        ...
        # Read vocabulary
        self.vocab = VOCAB_SET #{x.split()[0].lower() for x in self._read_s3_file(VOCAB_FILE).split("\n")}
        ...
        for (i, doc_key) in enumerate(doc_keys_to_process):
    ...
    def example_sentences_to_json(self):
        ...
        for key, heap in self.example_sentences.items():
            ...
            result[key] = [json.loads(x[1]) for x in heap]
